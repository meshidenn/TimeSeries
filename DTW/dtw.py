...
def calc_dtw(A, B):
    ...
    m[0][0] = (np.linalg.norm(A[0], B[0]), (-1, -1))
    for i in range(1, S):
        m[i][0] = (m[i-1][0][0] + np.linalg.norm(A[i], B[0]), (i-1, 0))
    for j in range(1, T):
        m[0][j] = (m[0][j-1][0] + np.linalg.norm(A[0], B[j]), (0, j-1))

    for i in range(1, S):
        for j in range(1, T):
            minimum, index = minVal(m[i-1][j], m[i][j-1], m[i-1][j-1])
            indexes = [(i-1, j), (i, j-1), (i-1, j-1)]
            m[i][j] = (extract_value(minimum) + np.linalg.norm(A[i], B[j]), indexes[index])
    return m
